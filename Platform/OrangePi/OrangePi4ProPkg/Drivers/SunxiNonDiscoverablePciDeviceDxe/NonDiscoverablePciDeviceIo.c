...
//
// Allwinner A733 PCIe cpu_pcie_addr_quirk: the DesignWare RC has an
// HW-fixed 0x20000000 offset between the AXI/CPU address space and the
...
//
// We only apply the offset when HostAddress lies in DRAM (>= 0x40000000)
// to avoid mangling MMIO addresses or addresses already below the offset.
//
#define SUNXI_PCIE_CPU_BASE_OFFSET  0x20000000ULL
#define SUNXI_PCIE_DRAM_BASE        0x40000000ULL
  ...
  )
{
  ...
  // PciIo provider for ALL NonDiscoverable devices — including the EHCI USB
  // controllers, which DMA identity-mapped. Applying the PCIe offset to USB
  // pointed the EHCI queue heads at the wrong DRAM (Buf 0x7FFFF000 ->
  // PhyAddr 0x5FFFF000) so every transfer timed out (SET_ADDRESS failed),
  // which broke the USB keyboard that worked in v0.2.
  //
  // When PCIe/NVMe is revived, re-apply this offset ONLY on the PCIe device's
